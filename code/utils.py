...
def load_data(path="../data/cora/", dataset="cora"):
    ...
    for i in range(layers):
        ...
        adj = load_in_adj(i, idx_map, path, dataset)
    ...
    for i in range(layers):
        for j in range(i+1,layers):
            bet_adj = load_bet_adj(i, j, idx_maps[i], idx_maps[j], path, dataset)
    ...
    return adjs, adjs_sizes, adjs_pos_weights, adjs_norms, bet_pos_weights, bet_norms, bet_adjs, bet_adjs_sizes,\
           features, features_sizes, labels, labels_nclass
